...
class TestFixtureRabin : public ::testing::Test {
  protected:
    ...
    const std::vector<Mpz> expected_ciphertext_output = {Mpz("2413364968701008174673980317676540920987654321")};
...
// Decryption test
TEST_F(TestFixtureRabin, Decrypt) { EXPECT_EQ(plaintext_input, crypto->decrypt(expected_ciphertext_output)); }

// Decode test
TEST_F(TestFixtureRabin, Decode) { EXPECT_FALSE(CryptoBase::decimal_to_english(plaintext_input).empty()); }